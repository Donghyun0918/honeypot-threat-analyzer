// ─────────────────────────────────────────────────────────────────────────────
// [정사평 패치] 원본: T-Pot attack map (upstream).
//
// CARTO 는 basemap 타일에 API 키를 요구하도록 바뀌었고, 키 없이 요청하면
// "API KEY REQUIRED" 워터마크가 인쇄된 타일을 HTTP 200 으로 돌려준다.
// 실패로 보이지 않아 더 나쁘다 — 지도는 뜨는데 글자만 덮여 있다.
//
// 키가 필요 없는 OSM 표준 타일로 바꾸고, 어두운 톤은 타일에 CSS 필터를
// 걸어 만든다. 라이선스 표기는 OpenStreetMap 만 남긴다.
// ─────────────────────────────────────────────────────────────────────────────
(function 다크타일필터() {
    var st = document.createElement('style');
    st.textContent =
        '.tile-dark { filter: invert(1) hue-rotate(185deg) brightness(0.94) contrast(0.86) saturate(0.5); }';
